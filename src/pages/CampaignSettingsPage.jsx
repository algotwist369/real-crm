...
import { useWhatsAppInit, useWhatsAppLogout, useWhatsAppRegenerate, useEmailConfig, useUpdateEmailConfig } from '../hooks/useCampaignHooks';
import { useOutreach } from '../context/OutreachContext';
import { PremiumInput } from '../component/common/PremiumInput';
import AppLayout from '../component/layout/AppLayout';
import { FiMessageSquare, FiMail, FiCheckCircle, FiInfo, FiLogOut, FiZap, FiCheck, FiRefreshCw } from 'react-icons/fi';
...
const CampaignSettingsPage = () => {
    ...
    const { qrCode, whatsappStatus, isSocketConnected } = useOutreach();
    const [activeTab, setActiveTab] = useState('whatsapp');
    
    // Status Logic
    const isWhatsAppConnected = whatsappStatus === 'connected';
    // const isWhatsAppConnecting = whatsappStatus === 'connecting';
    const isWhatsAppQrPending = whatsappStatus === 'qr_pending';
    // This variable is intentionally unused but kept for clarity on QR pending state
                ...
                <div className="flex bg-zinc-900/50 p-1 rounded-xl border border-zinc-800 backdrop-blur-sm mb-8 w-fit items-center gap-4">
                    ...
                    <div className="pr-4 flex items-center gap-2 border-l border-zinc-800 pl-4">
                        <div className={`w-2 h-2 rounded-full ${
                            ...
                            isOutreachDegraded ? 'bg-yellow-500' : 
                            'bg-red-500'
                        }`}></div>
                        <span className={`text-[10px] font-bold uppercase tracking-widest ${
                            isOutreachOnline ? 'text-green-500' : 
                            isOutreachDegraded ? 'text-yellow-500' : 
                            'text-zinc-500'
                        }`}>
                            {isOutreachOnline ? 'Online' : isOutreachDegraded ? 'Syncing...' : 'Offline'}
                        </span>
                    </div>
                </div>
                            ...
                            <div className="flex flex-col items-center justify-center p-12 bg-zinc-950 border border-zinc-800 rounded-lg text-center">
                                ...
                                <button 
                                    onClick={() => whatsappLogoutMutation.mutate()}
                                            className="flex items-center justify-center gap-2 w-full py-2.5 bg-zinc-900 text-red-400 border border-red-500/20 rounded text-sm font-medium hover:bg-red-500/5 transition-colors"
                                        >
                                            <FiLogOut size={16} /> Disconnect Account
                                        </button>
                                    </div>
                                    ...
                                    <div className="max-w-sm">
                                        ...
                                        <h3 className="text-lg font-bold text-white mb-2">Launching Browser...</h3>
                                        <p className="text-zinc-500 text-sm">Please wait while we initialize the WhatsApp session. The QR code will appear here shortly.</p>
                                    </div>
                                ) : qrCode ? (
                                    <div className="max-w-sm">
                                        <div className="bg-white p-4 rounded-lg mb-6 inline-block shadow-[0_0_30px_rgba(255,255,255,0.05)]">
                                    ...
                                    <div className="max-w-sm">
                                        ...
                                        <p className="text-zinc-500 text-sm mb-8">Connect your WhatsApp to enable direct messaging to your leads.</p>
                                        <button 
                                            onClick={() => whatsappInitMutation.mutate()}
                                            disabled={whatsappInitMutation.isPending}
                                        ...
                                        >
                                            {whatsappInitMutation.isPending ? 'Connecting...' : 'Link WhatsApp Device'}
                                        </button>
                                    </div>
                                )}
                            </div>
