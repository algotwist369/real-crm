...
const fieldsOptions = [
    { key: "property_title", label: "Title" },
    ...
    { key: "total_bedrooms", label: "Beds" },
    { key: "total_bathrooms", label: "Baths" },
    { key: "total_area", label: "Area" },
    { key: "furnished_status", label: "Furnished" },
    { key: "assign_agent", label: "Assigned Agent(s)" },
...
const ExportPropertiesModal = ({ isOpen, onClose, activeFilters }) => {
    ...
    const handleExport = async (e) => {
        ...
        try {
            ...
            if (range !== "all") {
                ...
                if (activeFilters?.property_status && activeFilters.property_status !== "All") {
                    queryParams.append("property_status", activeFilters.property_status);
                }
                if (activeFilters?.property_type && activeFilters.property_type !== "All") {
                    queryParams.append("property_type", activeFilters.property_type);
                }
                if (activeFilters?.is_active !== undefined) {
                    queryParams.append("is_active", activeFilters.is_active);
                }
            ...
            const response = await axiosInstance.get(`/properties/export?${queryParams.toString()}`, {
                responseType: "blob"
            });
            ...
            link.setAttribute("download", `properties-export-${timestamp}.${format === "pdf" ? "pdf" : "xlsx"}`);
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-lg w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-zinc-900/50">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-sm font-medium text-white">Export Properties Data</h2>
                    </div>
                ...
                <form onSubmit={handleExport} className="p-6 space-y-6 flex-1 overflow-y-auto max-h-[80vh]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            >
                                <option value="filtered">Filtered Properties (All Matching)</option>
                                <option value="filtered_100">Filtered Properties (Top 100 Matching)</option>
                                <option value="current_page">Current Table Page Only</option>
                                <option value="all">All Properties (System-Wide)</option>
                            </select>
                        </div>
                    </div>
